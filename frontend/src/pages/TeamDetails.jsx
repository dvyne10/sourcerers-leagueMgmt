...
function TeamDetails() {
  ...
  const navigateUpdateTeam = () => {
    navigate(`/updateteam/${routeParams.teamid}`);
  };
      ...
      .then((data) => {
        if (data.requestStatus === "RJCT") {
          ...
          setTeamInfo({
            ...
            displayTurnOnLookingForPlayers:
              data.buttons.displayTurnOnLookingForPlayers,
            displayTurnOffLookingForPlayers:
              data.buttons.displayTurnOffLookingForPlayers,
            displayUnjoinButton: data.buttons.displayUnjoinButton,
            displayInviteToLeagueButton:
              data.buttons.displayInviteToLeagueButton,
            nsLeaguesUserIsAdmin: data.buttons.nsLeaguesUserIsAdmin,
            displayUninviteToLeagueButton:
              data.buttons.displayUninviteToLeagueButton,
  ...
  const handleUninvite = () => {
    if (teamInfo.pendingInviteRequestId !== "") {
      if (
        confirm(
          "Please confirm if you want to proceed with invite cancellation."
        )
      ) {
        fetch(`${backend}/cancelrequest/${teamInfo.pendingInviteRequestId}`, {
          ...
          .then((data) => {
            if (data.requestStatus === "RJCT") {
              ...
              setTeamInfo({
                ...teamInfo,
                displayInviteToLeagueButton: true,
                displayUninviteToLeagueButton: false,
                pendingInviteRequestId: "",
              });
            }
          })
    ...
    fetch(`${backend}/invitetoleague/${routeParams.teamid}`, {
      ...
      .then((data) => {
        if (data.requestStatus === "RJCT") {
          ...
          setTeamInfo({
            ...teamInfo,
            displayInviteToLeagueButton: false,
            displayUninviteToLeagueButton: true,
            pendingInviteRequestId: data.pendingInviteRequestId,
          });
        }
      });
    setShowInvite(false);
      ...
      fetch(`${backend}/lookingforplayerson/${routeParams.teamid}`, {
        ...
        .then((data) => {
          if (data.requestStatus === "RJCT") {
            ...
            setTeamInfo({
              ...teamInfo,
              displayTurnOnLookingForPlayers: false,
              displayTurnOffLookingForPlayers: true,
            });
          }
        });
    }
      ...
      fetch(`${backend}/lookingforplayersoff/${routeParams.teamid}`, {
        ...
        .then((data) => {
          if (data.requestStatus === "RJCT") {
            ...
            setTeamInfo({
              ...teamInfo,
              displayTurnOnLookingForPlayers: true,
              displayTurnOffLookingForPlayers: false,
            });
          }
        });
    }
  ...
  const handleUnjoin = () => {
    if (
      confirm(
        `Please confirm if you want to leave the team ${teamInfo.details.teamName}.`
      )
    ) {
          ...
          if (data.requestStatus === "RJCT") {
            ...
            if (teamInfo.buttons.displayTurnOnLookingForPlayers) {
              setTeamInfo({
                ...
                displayUnjoinButton: false,
                displayCancelReqButton: false,
                pendingJoinRequestId: "",
                playerCurrentTeamName: "",
              });
            } else {
  ...
  const handleCancelRequest = () => {
    if (
      confirm(
        `Please confirm if you want to proceed with cancelling your request`
      )
    ) {
      ...
      .then((data) => {
        if (data.requestStatus === "RJCT") {
          ...
          setTeamInfo({
            ...
            displayJoinButton: false,
            displayUnjoinButton: false,
            displayCancelReqButton: true,
            pendingJoinRequestId: data.pendingJoinRequestId,
          });
        }
      });
    setShow(false);
  ...
  return (
    <div key={teamInfo.teamName}>
      {teamInfo.displayUpdateButton && (
        <div className="d-flex w-100 position-absolute justify-content-end p-4">
          <Button
            variant="transparent"
            onClick={navigateUpdateTeam}
      ...
      <div className="bg-light container justify-content-center text-center">
        ...
        <div
          ...
          style={{
            backgroundImage: `url(${backendPhotos}/teambanners/${teamInfo.details._id}.jpeg), url(${backendPhotos}/teambanners/default.jpg)`,
          ...
          >
            <Row>
              <Col lg="2" className="text-center">
                <p>
                  <strong>{teamInfo.details.location}</strong>
                </p>
                <Image
                  src={`${backendPhotos}/teamlogos/${teamInfo.details._id}.jpeg`}
              ...
              <Col>
                <h1>{teamInfo.details.teamName}</h1>
                <p className="mt-2">
                  <strong>Description</strong> : {teamInfo.details.description}
                </p>
                <h3 className="mt-5">Contact</h3>
                <p>
                  <a
                    href={`mailto:${teamInfo.details.teamContactEmail}`}
                    className="general-link-no-dec text-white text-decoration-underline"
                  >
                    {teamInfo.details.teamContactEmail}
                  </a>
                </p>
              </Col>
            </Row>
              ...
              <Col lg="2" className="mt-2">
                <>
                  {isSignedIn && teamInfo.displayInviteToLeagueButton && (
                    <Button
                      className="mt-2 mb-2 btn-success rounded-pill"
                  ...
                  {isSignedIn && teamInfo.displayUnjoinButton && (
                    <Button
                      className="mt-2 mb-2 btn-danger rounded-pill"
                  ...
                  {isSignedIn && teamInfo.displayTurnOnLookingForPlayers && (
                    <Button
                      className=""
                      onClick={handleTurnOnLookingForPlayers}
                    >
                      Turn on looking for players
                    </Button>
                  )}
                  {isSignedIn && teamInfo.displayTurnOffLookingForPlayers && (
                    <Button
                      className="mt-2 mb-2 btn btn-outline-dark rounded-pill"
                      onClick={handleTurnOffLookingForPlayers}
                    >
                      Turn off looking for players
                    </Button>
                  )}
                </>
              </Col>
              ...
              >
                <Row className="align-items-center">
                  <Col
                    sm={3}
                    href="www.google.com"
                    className="text-center rounded-start "
                    style={{ backgroundColor: "#C7DDCC" }}
                  >
                    <a
                      href={`/player/${player.playerId}`}
                      className="team-jersey-list-number"
                    >
                      <Image
                  ...
                  <Col md="auto" className="mx-auto mt-2">
                    ...
                    <h6>
                      {new Date(player.joinedTimestamp).toLocaleDateString(
                        "en-US"
                      )}
                    </h6>
                  </Col>
                      ...
                      {teamInfo.matches.map((match) => (
                        ...
                        >
                          <Row className="text-center">
                            ...
                            <Col md={3}>{match.team1.teamName}</Col>
                            <Col md={2}>
                              {match.team1.finalScore +
                                " - " +
                                match.team2.finalScore}
                            </Col>
                            <Col md={3}>{match.team2.teamName}</Col>
                            <Col md={2}>{match.locationOfMatch}</Col>
                          </Row>
                        </ListGroup.Item>
                      ))}
                    </ListGroup>
        ...
        <Modal show={show} onHide={handleClose}>
          ...
          <Modal.Body>
            <Form>
              ...
              >
                {teamInfo.playerCurrentTeamId !== "" ||
                  (teamInfo.playerCurrentTeamId !== undefined && (
                    <h4
                      style={{ color: "red" }}
                    >{`If you join this team, you will be removed from your current team ${teamInfo.playerCurrentTeamName}. Please confirm if you want to proceed.`}</h4>
                  ))}
                <Form.Label>
                  Explain shortly why you want to join this team.
                </Form.Label>
                <Form.Control
                  as="textarea"
                  rows={3}
        ...
        <Modal show={showInvite} onHide={handleCloseInvite}>
          ...
          <Modal.Body>
            <Form>
              ...
              <select
                id="leagueInvitedTo"
                name="leagueInvitedTo"
                className="form-control"
                value={leagueInvitedTo}
                onChange={handleLeagueInviteChange}
              >
                {teamInfo.nsLeaguesUserIsAdmin.map((option) => (
                  <option value={option.leagueId} key={option.leagueId}>
                    {option.leagueName}
                  </option>
                ))}
              </select>
              ...
              >
                <Form.Label>
                  Explain shortly why you want to invite this team to your
                  league.
                </Form.Label>
                <Form.Control
                  as="textarea"
                  ...
                  name="inviteMsg"
                  value={inviteMsg}
                  onChange={handleLeagueInviteMsg}
                />
              </Form.Group>
            </Form>
          </Modal.Body>
