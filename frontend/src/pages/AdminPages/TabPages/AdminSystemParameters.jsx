...
const AdminSystemParameters = () => {
      ...
      if (data.requestStatus === 'ACTC') {
        let parmList = data.details.map(parm =>{
          ...
          if (parm.parameterId === "sport") {
            preview = JSON.stringify(parm.sport)
          } else if (parm.parameterId === "statistic") {
            preview = JSON.stringify(parm.statistic)
          } else if (parm.parameterId === "position") {
            preview = JSON.stringify(parm.position)
          } else if (parm.parameterId === "login") {
            preview = JSON.stringify(parm.login)
          } else if (parm.parameterId === "dfltAnnouncement") {
            preview = JSON.stringify(parm.dfltAnnouncement)
          } else if (parm.parameterId === "maxParms") {
            preview = JSON.stringify(parm.maxParms)
  ...
  const columns = useMemo(() => [
      { accessorKey: 'parmId', header: 'Parameter Id', filterVariant: 'text', size: 50 },
      { accessorKey: 'parameterId', header: 'Parameter Type', filterVariant: 'select', size: 30 },
      { accessorKey: 'preview', header: 'Parameter Value (Preview)', filterVariant: 'text', size: 150 },
    ], [], );
  ...
  const handleGotoMntPage = (action, parmId) => {
    if (action === "CREATION") {
      ...
      navigate('/adminparmupdate/' + parmId)
    }
  }
    ...
    <div>
      ...
      <MaterialReactTable
        ...
        renderRowActionMenuItems={({ row }) => [
          <MenuItem key="edit" onClick={() => handleGotoMntPage("EDIT",  row.original.parmId)}>
            Edit
          </MenuItem>,
          <MenuItem key="delete" onClick={() => handleDeleteParameter(row.original.parmId)}>
            Delete
          </MenuItem>,
        ]}
